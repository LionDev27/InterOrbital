...
public class GridLogic : MonoBehaviour
{
    ...
    private void CreateRegions()
    {
        ...
        for (int i = 0; i < width; i++)
        {
            for (int j = 0; j < height; j++)
            {
                ...
                int tamRegion = 0; // Tamaño actual de la región
                RellenaRegion(i, j, biome, ref tamRegion); // Rellenar región a partir de la casilla actual
            }
        }*/
    }
    ...
    {
        if (x < 0 || x >= width || y < 0 || y >= height) return; // Verificar límites de la matriz
        if (_gridCells[x, y].biomeType != null || tamRegion >= 10) return; // Verificar si la casilla ya tiene un bioma asignado o si se alcanzó el tamaño máximo de región

        _gridCells[x, y].biomeType = bioma; // Asignar bioma a la casilla actual
        tamRegion++; // Incrementar tamaño de la región
